...
public class Conditions extends AbstractCondition {
    ...
    protected void onFirstSubscriber(Subscriber subscriber) {
        ...
        for (Condition condition : conditions) {
            condition.subscribe(new Subscriber() {
                @Override
                public void onSatisfied(Condition condition) {
                    if (satisfiedCount.incrementAndGet() == conditions.length) {
                        setSatisfied(true);
                    }
                }
                ...
                public void onUnsatisfied(Condition condition) {
                    int prevSatisfiedCount = satisfiedCount.getAndDecrement();
                    if (prevSatisfiedCount == conditions.length) {
                        setSatisfied(false);
                    }
                }
            });
